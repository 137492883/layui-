...
layui.define(['element', 'common'], function (exports) {
            ...
            _con.children('ul').find('li').each(function () {
                var $this = $(this);
                    var $dd = $this.each(function () {
                        $(this).on('mouseenter', function (data) {
                            if($('.layadmin-side-shrink').length  == 0 ){
                                return;
                            }
                            $('#leftFBar').css('top',($(this).index() * 60 + 80) +'px')
                            $('#leftFBar').show();
                            var $a = $(this).children('a');
                            var href = $a.data('url');
                            var icon = $a.children('i:first').data('icon');
                    ...
                    $('.layui-header').on('mouseenter', function () {
                        $('#leftFBar').hide();
                    })
                    $('#admin-body').on('mouseenter', function () {
                        $('#leftFBar').hide();
                    })
            });
        }
    ...
    function getHtml(data) {
        if (data.json){
            ...
            var ulHtml = '<ul class="layui-nav   beg-navbar">';
        }
            ...
            if (data[i].children !== undefined && data[i].children !== null && data[i].children.length > 0) {
                ...
                if (data[i].icon !== undefined && data[i].icon !== '') {
                    if (data[i].icon.indexOf('fa-') !== -1) {
                        ulHtml += '<i class="fa ' + data[i].icon + '" aria-hidden="true" data-icon="' + data[i].icon + '" data-id="' + data[i].id + '"></i>';
                    } else {
                        ulHtml += '<i class="layui-icon" data-icon="' + data[i].icon + '" data-id="' + data[i].id + '">' + data[i].icon + '</i>';
                    }
                }
                ...
                for (var j = 0; j < data[i].children.length; j++) {
                    ...
                    if (data[i].children[j].icon !== undefined && data[i].children[j].icon !== '') {
                        if (data[i].children[j].icon.indexOf('fa-') !== -1) {
                            ulHtml += '<i class="fa ' + data[i].children[j].icon + '" data-icon="' + data[i].children[j].icon + '" data-id="' + data[i].id + '" aria-hidden="true"></i>';
                        } else {
                            ulHtml += '<i class="layui-icon" data-icon="' + data[i].children[j].icon + '" data-id="' + data[i].children[j].id + '">' + data[i].children[j].icon + '</i>';
                        }
                    }
